...
/* loaded from: C:\Users\MaOk\Desktop\ADB\platform-tools-latest-windows\platform-tools\app-source3\base\smali_classes3\kotlinx\coroutines\internal\SegmentQueue.smali */
public abstract class SegmentQueue<S extends Segment<S>> {
    ...
    protected final S getSegment(S startFrom, long id) {
        ...
        S s = startFrom;
        while (s.getId() < id) {
            Segment next2 = s.getNext();
            if (next2 == null) {
                Segment segmentNewSegment = newSegment(s.getId() + 1, s);
                if (s.casNext(null, segmentNewSegment)) {
                    if (s.getRemoved()) {
                        s.remove();
                    }
                    moveTailForward(segmentNewSegment);
                    next = segmentNewSegment;
                } else {
                    next = s.getNext();
                    if (next == null) {
                        Intrinsics.throwNpe();
