...
public final class DialogFragmentNavigator extends Navigator<Destination> {
        ...
        if (savedState != null) {
            ...
            for (int index = 0; index < this.mDialogCount; index++) {
                ...
                if (fragment != null) {
                    ...
                    this.mRestoredTagsAwaitingAttach.add(DIALOG_TAG + index);
                }
            }
        }
    }
    ...
    void onAttachFragment(Fragment childFragment) {
        boolean needToAddObserver = this.mRestoredTagsAwaitingAttach.remove(childFragment.getTag());
        if (needToAddObserver) {
            childFragment.getLifecycle().addObserver(this.mObserver);
        }
    }

    /* loaded from: C:\Users\MaOk\Desktop\ADB\platform-tools-latest-windows\platform-tools\app-source3\base\smali\androidx\navigation\fragment\DialogFragmentNavigator$Destination.smali */
